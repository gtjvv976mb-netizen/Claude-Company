...
function render() {
  ...
  if (R) {
    $("record").innerHTML = `
      ...
      <p>Every modelled exit ladder over the 64 still loses (best: ${R.all64.bestModelledLadderSol} SOL against ${R.all64.netSol} actual). Nothing measured at entry orders the outcome. This lane waits ${Math.round((s.entryWaitMs ?? 0) / 1000)}s and buys only a launch that still marks ≥${s.entryFollowThroughX}x — a rule that keeps it out of the bucket that never won, not evidence of an edge. The shadow book grades it; export it and run <code>executor/grade-entry-gates.mjs --file</code>.</p>`;
  }
  $("log").innerHTML = (s.log ?? []).slice(0, 10).map((l) => `<div>${esc(new Date(l.at).toISOString().slice(11, 19))} ${esc(l.line)}</div>`).join("");
...
$("btnExport").addEventListener("click", async () => {
  ...
  a.href = url; a.download = `hawk-shadow-${new Date().toISOString().slice(0, 10)}.jsonl`;
  document.body.appendChild(a); a.click(); a.remove();
  setTimeout(() => URL.revokeObjectURL(url), 5_000);
  toast(`${res.rows} rows exported — grade them with executor/grade-entry-gates.mjs --file`);
});
