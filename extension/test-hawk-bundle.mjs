 ...
 *
 * esbuild is a devDependency of extension/. On GitHub Actions and Render it is installed
 * (`npm ci --prefix extension --ignore-scripts`); locally, a checkout without it skips
 * with a line that says so, and CI is where the claim is held.
 */
...
if (!esbuildPresent) {
  if (process.env.GITHUB_ACTIONS === "true" || process.env.RENDER) {
    console.log("  FAIL esbuild is not installed on CI — run `npm ci --prefix extension --ignore-scripts` in the workflow");
    process.exit(1);
  }
  console.log("  (esbuild is not installed here — `npm ci` in extension/ to run the bundle test; CI runs it)");
  process.exit(0);
}

console.log("\nTHE SHIMS AGREE WITH WHAT THEY REPLACE\n─────────────────────────────────────");
{
  const real = await import("../executor/jupiter.mjs");
  const shim = await import("./src/shims/jupiter.mjs");
  ok("WSOL matches jupiter.mjs", shim.WSOL === real.WSOL, shim.WSOL);
  ok("ATA_PROGRAM matches jupiter.mjs", shim.ATA_PROGRAM === real.ATA_PROGRAM, shim.ATA_PROGRAM);
...
if (built) {
  ...
  for (const stat of ["manifest.json", "popup.html", "popup.css", "options.html", "icons/hawk-32.png", "icons/hawk-128.png", "icons/hawk-512.png"])
