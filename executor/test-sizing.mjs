...
const p1 = planEntry({ call, cfg: DEFAULTS, state: st() });
...
const p2 = planEntry({ call: wide, cfg: DEFAULTS, state: st() });
...
const p3 = planEntry({ call: wide, cfg: DEFAULTS, state: st({wins:20, losses:10}) });
...
const p4 = planEntry({ call: wide, cfg: DEFAULTS, state: st({wins:3, losses:29}) });
t("hit rate under W_min is refused", p4.action==="skip" && /under the/.test(p4.reason), p4.reason);

// book heat
const p5 = planEntry({ call: wide, cfg: DEFAULTS, state: st({bookHeat: 0.079}) });
