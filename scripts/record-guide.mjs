/* THE GUIDE, RECORDED FROM THE REAL SITE (owner, 2026-09-13).
 *
 * Records the published pages — the tower, a lease card, the house floor, the Overview's
 * book, the calls, the bot, the Team tab's two tracks and BIG C's wall — as real clicks
 * on the live site, with a caption bar drawn over the page, then speaks the narration
 * with Piper and burns it into an H.264/AAC file the GUIDE tab plays. Writes:
 *
 ...
 *
 * Both are committed like the other finished media, so `npm run build` needs none of
 ...
 *
 * The holds below are sized to the narration: each chapter's wait is at least as long
 * as its spoken line, measured after synthesis, so the voice never runs into the next
 * chapter. Change the words and the waits are re-measured on the next run.
 */
...
export const CHAPTERS = [
  { id: "tower", title: "The tower", say: "This is Claude Tower on Solana. Fifty floors, one desk each. The house desk sits upstairs on floor fifty and publishes the coins it would trade." },
  { id: "lease", title: "Lease a floor", say: "Pick any vacant floor. Here is what it costs, one floor per wallet. And beside it, the house bot's real record, with the house's own money." },
  { id: "enter", title: "Watch HQ live", say: "Watch H Q live. The house floor is open to everyone, no sign-in needed." },
  { id: "overview", title: "The Overview", say: "The Overview. The bot's profit and loss from its own journal, what it holds right now, and every closed trade, each drawn as a bar from the stop to the target." },
  { id: "calls", title: "The calls", say: "Calls. Everything the desk published, with an entry, a stop and a target. These are the calls your bot takes." },
  { id: "wallste", title: "WALL-ST-E", say: "Wall Street E is the bot. Real money, your own caps, on your own machine." },
  { id: "team", title: "Your track", say: "Team. The sixteen analysts and the boss who work this floor. On a floor you lease, this tab is also where you choose who runs your bot: yourself, with one command on a Mac or a small Linux server, and the key made on your machine." },
  { id: "board", title: "Big C's board", say: "And on the wall, Big C's board keeps the live book, redrawn every ten seconds." },
...
const VACANT = await (async () => {
  try {
    const r = await fetch((process.env.GUIDE_API || "https://claude-company-api.onrender.com") + "/api/tower/floors");
...
const clickTab = (dest) => page.evaluate((d) => document.querySelector(`.dtab[data-destination="${d}"]`)?.click(), dest);
